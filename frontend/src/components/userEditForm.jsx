...
const [userId, setUserId] = useState("");
  ...
  const updateUserMutation = useMutation({
    mutationFn: async ({ userId: id, values }) => {
      await fetch(`${REACT_APP_BASE_URL}/user-update/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (userId) {
        updateUserMutation.mutate({ id: userId, values });
        formik.resetForm();
        formik.errors = {};
        return;
      } else {
  ...
  return (
    <div className="border p-5 rounded-4 flex justify-center">
      ...
      <Home formik={formik} setUserId={setUserId} />
    </div>
  );
}
